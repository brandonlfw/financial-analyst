...
def process_statement(csv_file, start_date, end_date, provinces, cities):
    ...
    account_type = statement_df["Account Type"].iloc[0] if "Account Type" in statement_df.columns and not statement_df.empty else None
    account_number = statement_df["Account Number"].iloc[0] if "Account Number" in statement_df.columns and not statement_df.empty else None

    statement_df = statement_df.drop(columns=["Cheque Number", "USD$", "Account Type", "Account Number"], errors="ignore")
    ...
    insights["account_type"] = account_type
    insights["account_number"] = account_number
